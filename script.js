...
function initMap() {
    ...
    google.maps.event.addListener(
        map,
        "bounds_changed",() => {
            fetch(`http://api.openweathermap.org/data/2.5/box/city?appid=3547b1c98b08942e241562ee9aaf3b30&bbox=${map.getMapBbox()}`)
            .then(function(response) {
                response.json().then(function(data) {
                  if (data.list) {
                      if (!downloadedCities.includes(data.list[0].name)) {
                        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${data.list[0].name}&appid=3547b1c98b08942e241562ee9aaf3b30`)
                        .then(function(res) {
